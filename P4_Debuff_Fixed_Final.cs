...
public class P4_Debuff_Reminder_Fixed : SplatoonScript<P4_Debuff_Reminder_Fixed.Config>
{
    ...
    private Dictionary<ulong, bool> IsLieByPlayer = [];
    ...
    public override void OnActionEffectEvent(ActionEffectSet set)
    {
        if(set.Action != null &&
           set.Source != null &&
           IsTruth.TryGetValue(set.Source.ObjectId, out var truth))
        {
            var isLie = !truth;
            foreach(var targetEffect in set.TargetEffects)
            {
                IsLieByPlayer[targetEffect.TargetID] = isLie;
            }
        }
    }
    ...
    public override void OnGainBuffEffect(uint sourceId, FFXIVClientStructs.FFXIV.Client.Game.Status Status)
    {
        ...
        {
            if(!IsLieByPlayer.TryGetValue(sourceId, out var isLieForSource))
            {
                return;
            }
            ...
            if(pc.AddressEquals(BasePlayer))
            {
                if(!C.ShowOthers)
                {
                    if((Debuffs.DebuffSpread.Contains(Status.StatusId) && !isLieForSource) || (Debuffs.DebuffStack.Contains(Status.StatusId) && isLieForSource))
                    {
                    ...
                    if(Debuffs.DebuffLookAway.Contains(Status.StatusId))
                    {
                        if(Status.RemainingTime > 65f)
                        {
                            if(C.OutputInChat)
                            {
                                Print(!isLieForSource ? UIColor.Red : (UIColor)16, isLieForSource ? C.LongGazeInv.Get() : C.LongGaze.Get());
                            }
                        }
                        else
                        {
                            if(C.OutputInChat)
                            {
                                Print(!isLieForSource ? UIColor.Red : (UIColor)16, isLieForSource ? C.ShortGazeInv.Get() : C.ShortGaze.Get());
                            }
                        }
                    }
                    ...
                    if(Debuffs.DebuffDontMove.Contains(Status.StatusId))
                    {
                        if(C.OutputInChat)
                        {
                            Print(!isLieForSource ? UIColor.Yellow : (UIColor)506, isLieForSource ? C.AccelerationBombInv.Get() : C.AccelerationBomb.Get());
                        }
                    }
                }
                ...
                if(Debuffs.DebuffFireSpread.Contains(Status.StatusId))
                {
                    if(C.OutputInChat)
                    {
                        Print(!isLieForSource ? (UIColor)557 : (UIColor)563, isLieForSource ? C.FireIsDonut.Get() : C.FireIsAOE.Get());
                    }
                }
                ...
                if(Debuffs.DebuffDonut.Contains(Status.StatusId))
                {
                    if(C.OutputInChat)
                    {
                        Print(!isLieForSource ? (UIColor)553 : (UIColor)555, isLieForSource ? C.WaterIsAOE.Get() : C.WaterIsDonut.Get());
                    }
                }
            }
            if(C.ShowOthers)
            {
                ...
                if(Debuffs.DebuffLookAway.Contains(Status.StatusId))
                {
                    ...
                    else
                    {
                        if(C.OutputInChat)
                        {
                            enqueue(!isLieForSource ? UIColor.Red : (UIColor)16, (isLieForSource ? C.Other_ShortGazeInv.Get() : C.Other_ShortGaze.Get()), $"{pc.Name}");
                        }
                    }
                }
                ...
                if(Debuffs.DebuffDontMove.Contains(Status.StatusId))
                {
                    if(C.OutputInChat)
                    {
                        enqueue(!isLieForSource ? UIColor.Yellow : (UIColor)506, (isLieForSource ? C.Other_AccelerationBombInv.Get() : C.Other_AccelerationBomb.Get()), $"{pc.Name}");
                    }
                }
            }
        }
